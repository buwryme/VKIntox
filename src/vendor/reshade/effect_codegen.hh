...
namespace reshadefx
{
        ...
        public:
                ...
                /// <summary>
                /// Look up an existing struct definition. Throws if not found,
                /// so callers don't dereference a past-end iterator (which was
                /// the previous behavior — UB on malformed input).
                /// </summary>
                ...
                struct_info &find_struct(id id)
                {
                        const auto it = std::find_if(_structs.begin(), _structs.end(),
                                [id](const auto &s) { return s.definition == id; });
                        if (it == _structs.end())
                                throw std::runtime_error("find_struct: unknown struct id " + std::to_string(id));
                        return *it;
                }
                /// <summary>
