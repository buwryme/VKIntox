...
#include "effect_lexer.hh"
#include "effect_preprocessor.hh"
#include <cassert>
#include <algorithm>
#include <stdexcept>
...
void reshadefx::preprocessor::parse_def()
{
        ...
        if (_input_stack[_current_input_index].lexer->input_string()[macro_name_end_offset] == '(')
        {
                ...
                if (accept(tokenid::ellipsis))
                {
                        ...
                        // Note: full variadic macro expansion is implemented in
                        // evaluate_identifier_as_macro — extra call arguments
                        // past the named parameters are joined with ", " and
                        // substituted as __VA_ARGS__.
                }
...
bool reshadefx::preprocessor::evaluate_identifier_as_macro()
{
        ...
        // Variadic macro support: if the macro is variadic and was invoked with
        // more arguments than named parameters, the trailing arguments are
        // joined with ", " and used as the value of __VA_ARGS__ (which is the
        // last entry in macro.parameters).
        if (it->second.is_variadic && !it->second.parameters.empty())
        {
                const size_t namedCount = it->second.parameters.size() - 1; // last is __VA_ARGS__
                if (arguments.size() > namedCount)
                {
                        std::string vaArgs;
                        for (size_t i = namedCount; i < arguments.size(); ++i)
                        {
                                if (i > namedCount)
                                        vaArgs += ", ";
                                vaArgs += arguments[i];
                        ...
                        arguments.push_back(std::move(vaArgs));
                }
                else if (arguments.size() == namedCount)
                {
                        // No variadic args supplied — __VA_ARGS__ expands to nothing.
