...
#include "effect_parser.hh"
#include "effect_codegen.hh"
#include <cassert>
        ...
        id   emit_load(const expression &exp, bool) override
        {
                ...
                for (; i < exp.chain.size(); ++i)
                {
                        ...
                        case expression::operation::op_member:
                                // Member access on an r-value (e.g. MyFunc().field,
                                // (cond ? a : b).field, MakeStruct(...).field). The
                                // OpAccessChain path above only fires for l-values,
                                // so we get here with `result` pointing at the whole
                                // composite. Extract the member with OpCompositeExtract.
                                result = add_instruction(spv::OpCompositeExtract, convert_type(op.to))
                                        .add(result)
                                        .add(op.index) // Member index
                                        .result;
                                break;
                        case expression::operation::op_dynamic_index:
                                ...
                                {
                                        // Dynamic column selection on a matrix r-value:
                                ...
                                // Array r-value indexed dynamically — we need a
                                // pointer to use OpAccessChain, so spill the value
                                // to a temporary function-scope variable first.
                                // This matches the parser's pattern for l-values.
                                {
                                        const spv::Id var_type = convert_type(op.from, true, spv::StorageClassFunction);
                                        const spv::Id var = add_instruction(spv::OpVariable, var_type, _current_function->variables)
                                                .add(spv::StorageClassFunction).result;
                                        _storage_lookup[var] = spv::StorageClassFunction;
                                        add_instruction_without_result(spv::OpStore)
                                                .add(var).add(result);
                                        const spv::Id ptr = add_instruction(spv::OpAccessChain, convert_type(op.to, true, spv::StorageClassFunction))
                                                .add(var).add(op.index).result;
                                        _storage_lookup[ptr] = spv::StorageClassFunction;
                                        result = add_instruction(spv::OpLoad, convert_type(op.to))
                                                .add(ptr).result;
                                }
                                break;
                        case expression::operation::op_constant_index:
                                assert(result != 0);
                                // Constant index into a composite r-value. Works for
                                // vectors, matrices, arrays, and structs.
                                result = add_instruction(spv::OpCompositeExtract, convert_type(op.to))
                                        .add(result)
